...
class RecentTransactionsCard extends GetView<DashboardController> {
  ...
  IconData _getCategoryIcon(String category) {
    ...
    if (category.contains('สื่อสาร') || category.contains('เน็ต')) return Icons.wifi_outlined;
    if (category.contains('เงินเดือน') || category.contains('Salary')) return Icons.account_balance_wallet_outlined;
    if (category.contains('ออม') || category.contains('DCA') || category.contains('กองทุน') || category.contains('Savings')) return Icons.savings_outlined;
    if (category.contains('ประกัน') || category.contains('สุขภาพ') || category.contains('Health')) return Icons.health_and_safety_outlined;
              ...
              ListView.separated(
                ...
                itemBuilder: (context, index) {
                  ...
                  if (item.isIncome) {
                    ...
                    prefix = '+';
                    natureLabel = 'INCOME';
                    natureColor = const Color(0xFF10B981);
                  } else if (item.isSavings) {
                    amountColor = const Color(0xFF3B82F6);
                    prefix = '';
                    natureLabel = 'SAVINGS';
                    natureColor = const Color(0xFF3B82F6);
                  } else {
                    amountColor = AppColors.nothingRed;
                    prefix = '-';
                    natureLabel = item.isFixedCost ? 'FIXED' : 'VAR';
                    natureColor = item.isFixedCost ? const Color(0xFF8B5CF6) : AppColors.nothingRed;
                  }
                  ...
                  return Dismissible(
                    ...
                    background: Container(
                      ...
                      decoration: BoxDecoration(
                        color: AppColors.nothingRed.withValues(alpha: 0.2),
                    ...
                    child: Material(
                      color: isDark ? const Color(0xFF0D0D0D) : const Color(0xFFF9F9F9),
                      borderRadius: BorderRadius.circular(14),
                      child: InkWell(
                        onTap: () {
                          HapticFeedback.lightImpact();
                            ...
                            children: [
                              ...
                              Container(
                                ...
                                decoration: BoxDecoration(
                                  ...
                                  border: Border.all(
                                    color: isDark ? AppColors.nothingBorder : Colors.black.withValues(alpha: 0.05),
                                    width: 0.8,
                                  ),
                                ),
                                child: Icon(
